...
Nodes whose global centrality exceeds the population distribution with high degree are classified as Epistemic Macro-Hubs:
\begin{equation}
...
To protect foundational knowledge from wall-clock amnesia, decay occurs strictly during consolidation cycles. Each node is assigned a load-bearing topological score:
\begin{equation}
\text{Score}_{\text{topology}}(v) = 0.4 \cdot (\pi^*(v) \cdot 10) + 0.3 \cdot \min(2.0, \text{deg}(v)/3) + 0.3 \cdot \text{Prior}_{\text{type}}(v)
\end{equation}
The effective decay rate is modulated by topology:
\begin{equation}
...
\begin{equation}
\text{Confidence}(v) \le 0.10, \quad \Delta t_{\text{dormant}} > 4, \quad \text{deg}(v) \le 1, \quad v \notin \mathcal{H}_{\text{macro}}, \quad \text{and} \quad \text{type}(v) \notin \{\text{identity}, \text{preference}\}
\end{equation}
...
\midrule
Category 1 (Factual Recall) & 22.84\% & 15.29\% & 2.81\% & \textbf{22.56\%} \\
...
Category 4 (Multi-Hop Inference) & 26.32\% & 31.58\% & 0.00\% & \textbf{23.68\%} \\
\bottomrule
\end{tabular}
\caption{Category-wise Recall@5 (\%) on LoCoMo.}
...
Comparing static RRF weights ($[0.35, 0.35, 0.35]$) against dynamic intent-routed weights demonstrates a +4.2\% improvement in Recall@5 on temporal queries and a +3.8\% improvement on multi-session queries, verifying that adapting weights based on query characteristics significantly enhances consensus ranking.
